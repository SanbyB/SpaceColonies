...
double SCALE_FACTOR = 1e-6; // Scale for rendering orbital distances (now a variable for zooming)
const double MIN_SCALE_FACTOR = 1e-8; // Zoomed all the way out
...
public:
    ...
    virtual void render(SDL_Renderer* renderer, Vector2D cameraOffset, double scaleFactor) {
        if (!texture) return;
        
        SDL_Rect destRect;
        destRect.x = static_cast<int>((position.x * scaleFactor) + (SCREEN_WIDTH / 2) - (size / 2) + cameraOffset.x);
        destRect.y = static_cast<int>((position.y * scaleFactor) + (SCREEN_HEIGHT / 2) - (size / 2) + cameraOffset.y);
        destRect.w = size;
        destRect.h = size;
...
public:
    ...
    void renderOrbit(SDL_Renderer* renderer, Vector2D cameraOffset, double scaleFactor) {
        ...
        int centerX = static_cast<int>((position.x * scaleFactor) + (SCREEN_WIDTH / 2) + cameraOffset.x);
        int centerY = static_cast<int>((position.y * scaleFactor) + (SCREEN_HEIGHT / 2) + cameraOffset.y);
        
        // Draw a circle to represent the gravitational influence
        int radius = static_cast<int>(this->radius * scaleFactor / 10);
...
public:
    ...
    void renderTrail(SDL_Renderer* renderer, Vector2D cameraOffset, double scaleFactor) {
        ...
        for (size_t i = 1; i < orbitTrail.size(); i++) {
            SDL_RenderDrawLine(
                renderer,
                static_cast<int>((orbitTrail[i-1].x * scaleFactor) + (SCREEN_WIDTH / 2) + cameraOffset.x),
                static_cast<int>((orbitTrail[i-1].y * scaleFactor) + (SCREEN_HEIGHT / 2) + cameraOffset.y),
                static_cast<int>((orbitTrail[i].x * scaleFactor) + (SCREEN_WIDTH / 2) + cameraOffset.x),
                static_cast<int>((orbitTrail[i].y * scaleFactor) + (SCREEN_HEIGHT / 2) + cameraOffset.y)
            );
        }
    }
    
    void render(SDL_Renderer* renderer, Vector2D cameraOffset, double scaleFactor) override {
        // Render the trail first so spacecraft appears on top
        renderTrail(renderer, cameraOffset, scaleFactor);
        
        // Then render the spacecraft itself
        SpaceObject::render(renderer, cameraOffset, scaleFactor);
        
        // Render thrust if active
        if (thrustActive && fuel > 0) {
            SDL_SetRenderDrawColor(renderer, 255, 165, 0, 255); // Orange for thrust
            int shipX = static_cast<int>((position.x * scaleFactor) + (SCREEN_WIDTH / 2) + cameraOffset.x);
            int shipY = static_cast<int>((position.y * scaleFactor) + (SCREEN_HEIGHT / 2) + cameraOffset.y);
...
public:
    ...
    void zoomAt(double factor, Vector2D targetPos) {
        ...
        double worldX = (targetPos.x - SCREEN_WIDTH/2 - cameraOffset.x) / SCALE_FACTOR;
        double worldY = (targetPos.y - SCREEN_HEIGHT/2 - cameraOffset.y) / SCALE_FACTOR;
        
        // Apply zoom factor, clamping to min/max zoom levels
        double newScale = SCALE_FACTOR * factor;
    ...
    void render() {
        ...
        for (auto& body : celestialBodies) {
            body->renderOrbit(renderer, cameraOffset, SCALE_FACTOR);
            body->render(renderer, cameraOffset, SCALE_FACTOR);
        }
        
        // Render player spacecraft
        playerShip->render(renderer, cameraOffset, SCALE_FACTOR);
    ...
    void renderUI() {
        ...
        // Display zoom level
        // You'd need to implement text rendering to show this
    }
